...
public class WriteAxiomsToTextFiles {
	...
	public void writeAxioms(List<String> filePaths) throws OWLOntologyCreationException, IOException {
...
//      String filePath = "userInputCountOntology.owl";
		...
		//List<String> filePaths = List.of("UNIV-BENCH-OWL2DL.owl", "another-ontology.owl", "yet-another-ontology.owl");

		// String filePath = "UNIV-BENCH-OWL2DL.owl";
		// String ontoResult = readTxtFiles(filePath);
		OWLOntologyManager man = OWLManager.createOWLOntologyManager();
		// OWLOntology ontology = man.loadOntologyFromOntologyDocument(new
		// StringDocumentSource(ontoResult));
		...
		for (String filePath : filePaths) {
			try {
				...
				Set<OWLObjectPropertyDomainAxiom> objectDomainAxioms = ontology
						.getAxioms(AxiomType.OBJECT_PROPERTY_DOMAIN);
				Set<OWLObjectPropertyRangeAxiom> objectRangeAxioms = ontology
						.getAxioms(AxiomType.OBJECT_PROPERTY_RANGE);
				Set<OWLSubObjectPropertyOfAxiom> objectSubPropertyAxioms = ontology
						.getAxioms(AxiomType.SUB_OBJECT_PROPERTY);

				Set<OWLAsymmetricObjectPropertyAxiom> objectAsymmetricAxioms = ontology
						.getAxioms(AxiomType.ASYMMETRIC_OBJECT_PROPERTY);
				Set<OWLFunctionalObjectPropertyAxiom> objectFunctionalAxioms = ontology
						.getAxioms(AxiomType.FUNCTIONAL_OBJECT_PROPERTY);
				Set<OWLInverseFunctionalObjectPropertyAxiom> objectInverseFunctionalAxioms = ontology
						.getAxioms(AxiomType.INVERSE_FUNCTIONAL_OBJECT_PROPERTY);
				Set<OWLInverseObjectPropertiesAxiom> objectInverseAxioms = ontology
						.getAxioms(AxiomType.INVERSE_OBJECT_PROPERTIES);
				Set<OWLIrreflexiveObjectPropertyAxiom> objectIrreflexiveAxioms = ontology
						.getAxioms(AxiomType.IRREFLEXIVE_OBJECT_PROPERTY);
				Set<OWLReflexiveObjectPropertyAxiom> objectReflexiveAxioms = ontology
						.getAxioms(AxiomType.REFLEXIVE_OBJECT_PROPERTY);
				Set<OWLSymmetricObjectPropertyAxiom> objectSymmetricAxioms = ontology
						.getAxioms(AxiomType.SYMMETRIC_OBJECT_PROPERTY);
				Set<OWLTransitiveObjectPropertyAxiom> objectTransitiveAxioms = ontology
						.getAxioms(AxiomType.TRANSITIVE_OBJECT_PROPERTY);

				Set<OWLSubPropertyChainOfAxiom> objectPropertyChainAxioms = ontology
				...
				Set<OWLEquivalentObjectPropertiesAxiom> objectEquivalentAxioms = ontology
						.getAxioms(AxiomType.EQUIVALENT_OBJECT_PROPERTIES);
				Set<OWLDisjointObjectPropertiesAxiom> objectDisjointWithAxioms = ontology
				...
				Set<OWLFunctionalDataPropertyAxiom> dataFunctionalAxioms = ontology
						.getAxioms(AxiomType.FUNCTIONAL_DATA_PROPERTY);
				Set<OWLEquivalentDataPropertiesAxiom> dataEquivalentAxioms = ontology
						.getAxioms(AxiomType.EQUIVALENT_DATA_PROPERTIES);
				Set<OWLDisjointDataPropertiesAxiom> dataDisjointWithAxioms = ontology
				...
				Object[] cleanAxioms = { disjointUnionAxioms, objectDomainAxioms, objectSubPropertyAxioms,
						objectAsymmetricAxioms, objectFunctionalAxioms, objectInverseFunctionalAxioms,
						objectInverseAxioms, objectIrreflexiveAxioms, objectReflexiveAxioms, objectSymmetricAxioms,
						objectTransitiveAxioms, objectPropertyAxioms, objectEquivalentAxioms, dataDomainAxioms,
						dataSubPropertyAxioms, dataFunctionalAxioms, dataEquivalentAxioms, hasKeyAxioms };
				for (Object construct : cleanAxioms) {
					readAxiomsFromParticularConstruct((Set) construct);
				}

				Object[] restrictions = { subClassOfAxioms, equivalentClassAxioms };
				for (Object construct : restrictions) {
					readAxiomsForRestrictionsTypes((Set) construct);
				}

				Object[] moreThanThreeWordedAxioms = { objectDisjointWithAxioms, dataDisjointWithAxioms,
						disjointWithClassAxioms, owlClassAxioms, classAssertionAxioms, objectPropertyChainAxioms };
				for (Object construct : moreThanThreeWordedAxioms) {
					readAxiomsFromDisjointAndDeclaration((Set) construct);
				}

				Object[] rangesOnes = { dataRangeAxioms, objectRangeAxioms };
				for (Object construct : rangesOnes) {
					readAxiomsFromRangesOnes((Set) construct);
				}
			}

			catch (IOException | OWLOntologyCreationException e) {
				System.err.println("Error loading ontology from file: " + filePath);
				e.printStackTrace();
			}
		}
	}
	...
	public static void separateValuesOnlyFunction(String[] splited) throws IOException {
		...
		list.add(splited[1]);
		list.add(splited[3]);
		if (splited[2].contains("AllValues")) {
			list.add("only");
		} else if (splited[2].contains("SomeValues")) {
			list.add("some");
		}

		if (!splited[2].contains("HasSelf")) {
			list.add(splited[4]);
		}
		String axiom = "";
		...
		genericWriteToFile(axiom, fileMap.get(ontologyTerm.get(splited[2])),
				separatorMap.get(ontologyTerm.get(splited[2])));
	}
	...
	public static void readAxiomsForRestrictionsTypes(Set<OWLAxiom> constructAxioms) throws IOException {
		for (OWLAxiom ax : constructAxioms) {
			...
			if (splited.length > 3) {
				if (splited[2].contains("SomeValue") || splited[2].contains("AllValue")
						|| splited[2].contains("HasValue") || splited[2].contains("HasSelf")) {
					separateValuesOnlyFunction(splited);
				} else if (splited[2].contains("Cardinality")) {
					ArrayList<String> list = new ArrayList<String>();
					list.add(splited[1]);
					list.add(splited[4]);
					if (splited[2].contains("MinCardinality")) {
						list.add("min");
					} else if (splited[2].contains("MaxCardinality")) {
						list.add("max");
					} else {
						list.add("exactly");
					}
				...
				genericWriteToFile(axiom, fileMap.get(ontologyTerm.get(splited[0])),
						separatorMap.get(ontologyTerm.get(splited[0])));
			}
		}
	}
...
//    		System.out.println(Arrays.toString(splited)+" || "+splited.length);
	...
	// Some,HasVal,AllVale,Cardinality -> subclass and equivalent both data and
	// object
	// oneof, complement, intersection, union -> data ke liye data-ranges yeh fix
	// aur kahin nhi aayega
	...
	// ObjectPropertyRange hasMajor ObjectUnionOf Engineering FineArts
	// HumanitiesAndSocialScience Management Science
	// DataPropertyRange hasPublicationDate DataUnionOfxsd:dateTime
	// xsd:dateTimeStamp
	// DataPropertyRange hasAge DataComplementOfxsd:negativeInteger
	// DataPropertyRange hasPublicationDate DataUnionOfxsd:dateTime
	// xsd:dateTimeStamp

	// EquivalentClasses ScienceStudent ObjectSomeValuesFrom hasCollegeDiscipline
	// Science
	...
	// EquivalentClasses SemiGovernmentOrganization ObjectOneOf
	// NonProfitOrganization ProfitOrganization
	// EquivalentClasses SelfAwarePerson ObjectHasSelf knows
	// EquivalentClasses Organization ObjectUnionOf GovernmentOrganization
	// PrivateOrganization SemiGovernmentOrganization
...
//		test = test.replace("\"", "");
////		test = test.replace("(\"", " ");
//		test = test.replace("<http://kracr.iiitd.edu.in/OWL2Bench/", " "); // need to check what happens with other iris
//		test = test.replace(">", "");
//		test = test.replace("(", " ");
//		test = test.replace(")", "");
//		test = test.replace("^^", " ");
...
//		return result;
	    ...
	    // Extract the last part of any IRI after # or /
	...
	public static void mapping() throws IOException {
		...
		String[] filename1 = { "OwlClass.txt", "OwlObjectComplementFeature.txt", "OwlObjectIntersectionFeature.txt",
				"OwlObjectOneOfFeature.txt", "OwlObjectUnionOfFeature.txt" };
		String[] separator1 = { "\\r?\\n", " complementOf ", " IntersectionOf ", " OneOf ", " UnionOf " };
		String[] generatedOntologyTerm1 = { "", "ObjectComplementOf", "ObjectIntersectionOf", "ObjectOneOf",
				"ObjectUnionOf" };

		String[] construct2 = { "DisjointUnion", "DisjointWith", "EquivalentClass", "RdfsSubClassOf" }; // "AllDisjointClasses",
		String[] filename2 = { "OwlDisjointUnionFeature.txt", "OwlDisjointWithFeature.txt",
				"OwlEquivalentClassFeature.txt", "RdfsSubClassOfFeature.txt" }; // "OwlAllDisjointClassesFeature.txt",
		String[] separator2 = { " DisjointUnionOf ", " disjointWith ", " EquivalentClasses ", " SubClassOf " }; // " ,
																												// ",
		String[] generatedOntologyTerm2 = { "DisjointUnion", "DisjointClasses-DisjointWith", "EquivalentClasses",
		...
		String[] filename3 = { "OwlAsymmetricProperty.txt", "OwlEquivalentObjectProperty.txt",
				"OwlFunctionalObjectProperty.txt", "OwlInverseFunctionalProperty.txt", "OwlInverseOfProperty.txt",
				"OwlIrreflexiveProperty.txt", "OwlObjectPropertyDisjointWith.txt", "OwlReflexiveProperty.txt",
				"OwlSymmetricProperty.txt", "OwlTransitiveProperty.txt", "RdfsObjectDomain.txt", "RdfsObjectRange.txt",
				"OwlPropertyChainAxiomFeature.txt", "RdfsObjectSubPropertyOf.txt", "OwlObjectProperty.txt" }; // "AllDisjointObjectProperties.txt",
		String[] separator3 = { " Asymmetric", " equivalent ", " Functional", " inverse", " inverseOf ", " irreflexive",
				" disjointWithObject ", " reflexive", " Symmetrical", " Transitive", " domain ", " range ", " , ",
				" SubPropertyOf ", " " }; // " disjointObject ",
		String[] generatedOntologyTerm3 = { "AsymmetricObjectProperty", "EquivalentObjectProperties",
				"FunctionalObjectProperty", "InverseFunctionalObjectProperty", "InverseObjectProperties",
				"IrreflexiveObjectProperty", "DisjointObjectProperties-DisjointWith", "ReflexiveObjectProperty",
				"SymmetricObjectProperty", "TransitiveObjectProperty", "ObjectPropertyDomain", "ObjectPropertyRange",
				"SubObjectPropertyOfObjectPropertyChain", "SubObjectPropertyOf", "" }; // "DisjointObjectProperties",
		...
		String[] filename4 = { "OwlObjectAllValuesFromFeature.txt", "OwlObjectHasSelfFeature.txt",
				"OwlObjectHasValueFeature.txt", "OwlObjectSomeValuesFromFeature.txt",
				"OwlObjectQualifiedCardinalityFeature.txt", "OwlObjectMaxQualifiedCardinalityFeature.txt",
				"OwlObjectMinQualifiedCardinalityFeature.txt" };
		String[] separator4 = { " ", " ", " ", " ", " ", " ", " " };
		String[] generatedOntologyTerm4 = { "ObjectAllValuesFrom", "ObjectHasSelf", "ObjectHasValue",
				"ObjectSomeValuesFrom", "ObjectExactCardinality", "ObjectMaxCardinality", "ObjectMinCardinality" };
		...
		String[] filename5 = { "EquivalentDataProperty.txt", "OwlFunctionalDataProperty.txt",
				"DataPropertyDisjointWith.txt", "RdfsDataDomain.txt", "RdfsDataRange.txt",
				"RdfsDataSubPropertyOf.txt" }; // "AllDisjointDataProperties.txt",
		String[] separator5 = { " EquivalentTo ", " Functional", " disjointWithData ", " domain ", " range ",
				" subpropertyof " }; // " disjointData ",
		String[] generatedOntologyTerm5 = { "EquivalentDataProperties", "FunctionalDataProperty",
				"DisjointDataProperties-DisjointWith", "DataPropertyDomain", "DataPropertyRange", "SubDataPropertyOf" }; // "DisjointDataProperties",
		...
		String[] filename6 = { "DataAllValues.txt", "DataHasValue.txt", "DataMaxQualifiedCardinality.txt",
				"DataMinQualifiedCardinality.txt", "DataQualifiedCardinality.txt", "DataSomeValuesFrom.txt" };
		String[] separator6 = { " ", " ", " ", " ", " ", " " };
		String[] generatedOntologyTerm6 = { "DataAllValuesFrom", "DataHasValue", "DataMaxCardinality",
				"DataMinCardinality", "DataExactCardinality", "DataSomeValuesFrom" };
		...
		String[][] separator = { separator1, separator2, separator3, separator4, separator5, separator6, separator7,
				separator10 };
		String[][] generatedOntologyTerm = { generatedOntologyTerm1, generatedOntologyTerm2, generatedOntologyTerm3,
				generatedOntologyTerm4, generatedOntologyTerm5, generatedOntologyTerm6, generatedOntologyTerm7,
				generatedOntologyTerm10 };
		...
		for (int i = 0; i < construct.length; i++) {
			// System.out.println(construct[i].length+" | "+filename[i].length+" |
			// "+construct[i].length);
